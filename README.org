...
- ストアアプリとしてインストールすると、テキスト カーソル インジケータの色の変更の設定をしていても色は変わりません（ストアアプリがレジストリを変更できない制約のためです。GitHub Release からポータブルアプリをインストールすれば改善します）。
- Keyhac1 と Keyhac2 は同時に動作しません（キーボードフックを OFF にしても、同時利用はできません）。
- まだまだ不具合が潜在していると思われます。不具合を発見した場合は、Issue を登録してください。


[[/fakeymacs_logo/fakeymacs_logo.png]]

Fakeymacs2 は、Windows の操作を Emacs のキーバインドで行えるようにするための
Keyhac の設定です。

Keyhac for Windows v1.82 以上のバージョンでご利用ください。
Keyhac 2 でも動作するように調整をしていますが、まだ完全ではありません。
特に、拡張機能で動かないものがあります。対策を継続しています。

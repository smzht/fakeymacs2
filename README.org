...
- [[memo.org][Fakeymacs 2 memo]]

[[/fakeymacs_logo/fakeymacs2_logo.png]]

Fakeymacs 2 は、Windows の操作を Emacs のキーバインドで行えるようにするための
Keyhac の設定です。

Keyhac 1 の Keyhac for Windows v1.82 以上のバージョンもしくは Keyhac 2でご利用ください。

- https://sites.google.com/site/craftware/keyhac-ja
- https://crftwr.github.io/keyhac/

ドキュメントは次のリンクを参照してください。

1. [[fakeymacs_docs/configuration_parameters.org][Configuration Parameters]]
1. [[fakeymacs_docs/key_bindings.org][Key Bindings]]
1. [[fakeymacs_docs/extensions.org][Extensions]]
1. [[fakeymacs_docs/functions.org][Functions]]
...
Emacs 日本語入力モードになると Emacs キーバインドとして利用できるキーが限定され、その他のキーは
Windows にそのまま渡されるようになるため、IME のショートカットキーが利用できるようになります。
また、このモードでは IME のショートカットキーを置き換える機能もサポートしており、初期値では
「ことえり」のキーバインドを利用できるようにしています。
詳しくは、次のページの「■ 「Emacs 日本語入力モード」のキーバインド」の項を参照してください。

- [[/fakeymacs_docs/key_bindings.org#-emacs-日本語入力モードのキーバインド]]
...
[[/fakeymacs_docs/keymap_layer/keymap_layer.png]]
...
※ keymap_zed レイヤーは、zed_key Extension を有効にしたときに機能するレイヤーです。

※ keymap_obsidian レイヤーは、obsidian_key Extension を有効にしたときに機能するレイヤーです。

※ keymap_micro レイヤーは、micro_key Extension を有効にしたときに機能するレイヤーです。

※ keymap_fresh レイヤーは、fresh_key Extension を有効にしたときに機能するレイヤーです。

※ keymap_vim レイヤーは、vim_key Extension を有効にしたときに機能するレイヤーです。

※ keymap_elecxzy レイヤーは、elecxzy_key Extension を有効にしたときに機能するレイヤーです。

※ keymap_mc レイヤーは、mc_key Extension を有効にしたときに機能するレイヤーです。

※ keymap_yazi レイヤーは、yazi_key Extension を有効にしたときに機能するレイヤーです。
...
次のサイトから Keyhac 1 もしくは Keyhac 2 をインストールしてください。

- https://github.com/crftwr/keyhac-win/releases
- https://github.com/crftwr/keyhac/releases

※ Keyhac 1 利用の際、起動時に dll load エラーが発生する場合は、
「Microsoft Visual C++ 再頒布可能パッケージ」をインストールする必要があります。
次のサイトから最新の「Visual Studio 2015、2017、2019、および2022の Microsoft Visual C++
再頒布可能パッケージ」をダウンロードし、インストールしてください。
（Keyhac for Windows v1.82 を利用する場合は x86版 を、Keyhac for Windows v1.83
を利用する場合は、x64版 をインストールしてください。）

- https://docs.microsoft.com/ja-JP/cpp/windows/latest-supported-vc-redist?view=msvc-170

※ Keyhac 2 は、次のサイトから Microsoft Store アプリとしてインストールすることもできます。
ただし、この場合、Keyhac からテキスト カーソル インジケータの色を変更することができないなど、
幾つか制約が発生します。

- https://crftwr.github.io/keyhac/
- https://apps.microsoft.com/detail/9p8h1pg6prhh?hl=ja-JP&gl=JP
...
※ Keyhac 2 を Microsoft Store アプリとしてインストールした場合は、Keyhac 2 を最初に起動した後に
できる %USERPROFILE%\.keyhac フォルダに複写してください。
...
※ IME の Copilot Keyboard は、Keyhac の Window.getImeStatus() が正常に値を返さないため、現時点では
サポートしていません。このため、fc.ime でも Copilot Keyboard を選択することはできません。
...
スタートメニューからの起動（Microsoft Store アプリとしてインストールした場合）もしくは
keyhac.exe をダブルクリック（ポータブルアプリとしてインストールした場合）することで
Keyhac を起動させてください。

Keyhac を起動すると、タスクバーの ＾アイコンの中に Keyhac のアイコンが表示されます。
必要に応じて、通知領域に表示するように調整してください。
このアイコンを左クリックするとコンソールが表示され、右クリックすると機能の一覧が表示されます。

*** 11) レジストリ LowLevelHooksTimeout の設定を行う

Fakeymacs を利用して、文字が二重に入力されたり、Keyhac 1 のコンソールに「Time stamp inversion happened.」
...
と異なり、ターミナルのタイトルを置き換える機能を持っていません。
...
** Fresh Editor の対応について

Fakeymacs は、Fresh Editor に対応しています。Linux版 Micro Editor と同様、settitle
コマンド経由で Fresh Editor が起動するようにしてご利用ください。

#+BEGIN_EXAMPLE
alias fe='settitle fresh'
#+END_EXAMPLE

※ Fresh Editor の最新のバージョンでは、タイトルを置き換える機能をサポートしています。
しかし、エディタ終了時にタイトルを元に戻す機能は持っていないようですので、settitle と
組み合わせてご利用ください。

Fakeymacs では、ターミナルのタイトルに「- fresh」もしくは「— Fresh」が付いていることを
検知すると Emacs キーバインドにするようにしていますので、結果として Fresh Editor が
Emacs キーバインドで利用できるようになります。

Fresh Editor の Linux 版は、次のページからダウンロード可能です。

- https://getfresh.dev/

Fresh Editor の使い方は、以下が参考になると思います。

- https://github.com/sinelaw/fresh/blob/master/docs/USER_GUIDE.md

なお、Fresh Editor は、エディタ自体が Emacs キーバインドをサポートしています。
ただし、サポートされている機能が限定されていますので、Emacs ユーザは Fakeymacs の機能を
利用した方が違和感が少ないと思われます。

また、kill_line の機能（キーバインド C-k）は、Fresh Editor 側の C-k の機能を利用しています。
これは Fresh Editor で起動したターミナルで C-k が利用できるようにするための対策ですが、
この対策により、kill_line で切り取った文字列が Windows のクリップボードに格納されません。
これは、 Fresh Editor 利用する際の制約事項とさせていただければと思います。
クリップボードに格納させながら削除するには、カーソル位置から行末までをリージョンで選択し、
C-w でカットするようにしてください。

** Vim 系エディタの対応について

Fakeymacs は、以降に記載している vim_key 拡張機能を有効にすることで、Vim 系エディタを Emacs
キーバインドで利用可能とします。ノーマルモードとインサートモードのどちらでも Emacs キーバインドが
利用できる他、ノーマルモードでは Vim のほとんどのキーバインドがそのまま利用できますので、
Vim と Emacs のハイブリッドのようなエディタの操作感となります。
Vim のエキスパートの方には不要な拡張機能かもしれませんが、Emacsキーバインドユーザが環境条件により
Vim を使う必要がでた場合に、有用な拡張機能になるのではないかと思います。

なお、Vim のキーバインドについては、次の Vim Cheat Sheet を参照してください。

- https://gihyo.jp/assets/files/magazine/SD/2015/201510/download/Furoku_CheatSheet_Vim.pdf
...
- https://www.debian.org/doc/docs/debian-reference/ch01.ja.html#_midnight_commander_mc
...
「Edit=edit %f」の箇所は、「Edit=settitle micro %f」や「Edit=settitle fresh %f」などとすると、
先に紹介した Micro Editor や Fresh Editor などが Emacs キーバインドで利用できるようになります。
...
Zed エディタについては、次の拡張機能により、機能強化を図っています。

|----------------+----------------------------------|
| Extension name | Description                      |
|----------------+----------------------------------|
| [[/fakeymacs_extensions/zed_key][zed_key]]        | Zed エディタ用のキーの設定を行う |
|----------------+----------------------------------|
...
** Fresh Editor の機能追加について

Fresh Editor については、次の拡張機能により、キーバインドの追加設定を行っています。

|----------------+-----------------------------------|
| Extension name | Description                       |
|----------------+-----------------------------------|
| [[/fakeymacs_extensions/fresh_key][fresh_key]]      | Fresh Editor 用のキーの設定を行う |
|----------------+-----------------------------------|

** Vim 系エディタの機能追加について

Vim 系エディタについては、次の拡張機能により、キーバインドの設定を行っています。

|----------------+------------------------------------|
| Extension name | Description                        |
|----------------+------------------------------------|
| [[/fakeymacs_extensions/vim_key][vim_key]]        | Vim 系エディタ用のキーの設定を行う |
|----------------+------------------------------------|

** elecxzy エディタの機能追加について

elecxzy エディタについては、次の拡張機能により、キーバインドの設定を行っています。

|----------------+--------------------------------------|
| Extension name | Description                          |
|----------------+--------------------------------------|
| [[/fakeymacs_extensions/elecxzy_key][elecxzy_key]]    | elecxzy エディタ用のキーの設定を行う |
|----------------+--------------------------------------|

** Midnight Commander の機能追加について

Midnight Commander  については、次の拡張機能により、キーバインドの設定を行っています。
...
Yazi については、次の拡張機能により、キーバインドの設定を行っています。

|----------------+---------------------------|
| Extension name | Description               |
|----------------+---------------------------|
| [[/fakeymacs_extensions/yazi_key][yazi_key]]       | Yazi 用のキーの設定を行う |
|----------------+---------------------------|
...
- [[fakeymacs_docs/HHKB_Studio_keymap_sample/README.org][HHKB Studio keymap sample]]
...
● Keyhac 1 のクリップボードリスト画面で migemo 検索を可能とするためには、辞書ファイルを登録する必要
